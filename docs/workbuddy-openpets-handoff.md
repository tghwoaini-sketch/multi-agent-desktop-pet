...
- 进入 `review` 时，气泡标题会加上“需要你”，正文会明确显示“需要你的回复才能继续”；回复后自动恢复为 `running`。
- “本轮结束、可以继续输入”不能自动当成 `waiting`；如果本轮目标已经完成，应发送 `done`。
- 任务完成后不能继续复用旧的 `waiting` 气泡。
- 完成或失败只允许由已挂载的运行任务转换产生，并保持挂载；只有用户点击终态气泡后，才清理同一个 `threadId`。
...
6. 完成或失败只允许由运行态转换产生，历史终态不直接挂载；终态确认按任务 ID 永久持久化，不依赖会变化的标题、摘要或更新时间签名。同一任务即使状态抖动、桥接重启或再次被历史数据读取，也不能重新挂载，真正的新任务必须使用新的任务 ID；
7. 点击完成或失败气泡后清理该气泡、激活 WorkBuddy，且不留下空白网页；
8. 同时最多保留 5 个 WorkBuddy 任务气泡。
